...
class LazySupervisedGeoDataset(Dataset):
    """Dataset for supervised fine-tuning.
    This class is originally implemented by the LLaVA team and modified by
    Ji Lin and Haotian Tang.
    """
    ...
    def __init__(
        ...
        super(LazySupervisedGeoDataset, self).__init__()
        try:
            with open(data_path, "r") as fp:
                list_data_dict = json.load(fp)
        except:
    ...
    def __getitem__(self, i):
        ...
        if "image" in sources[0]:
            ...
            images, video_loading_succeed = self._load_video(video_path, num_video_frames, self.data_args, fps=fps,
        ...
        data_dict["image_num"] = image_num
        return data_dict

class LazySupervisedLevirDataset(Dataset):
    """Dataset for supervised fine-tuning.
    This class is originally implemented by the LLaVA team and modified by
    Ji Lin and Haotian Tang.
    """
    ...
    def __init__(
        ...
        super(LazySupervisedLevirDataset, self).__init__()
        try:
            with open(data_path, "r") as fp:
                list_data_dict = json.load(fp)
        except:
...
class LazySupervisedEraDataset(Dataset):
    """Dataset for supervised fine-tuning.
    This class is originally implemented by the LLaVA team and modified by
    Ji Lin and Haotian Tang.
    """
    ...
    def __init__(
        ...
        super(LazySupervisedEraDataset, self).__init__()
        try:
            with open(data_path, "r") as fp:
                list_data_dict = json.load(fp)
        except:
        ...
        self.data_args = data_args
        self.image_folder = image_folder
        self.image_num = self.data_args.num_video_frames
...
# TODO: To David: Check whether the program run well.
@dataclass
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]):
        ...
        input_ids, labels, images, image_num = [], [], [], []
        for instance in instances:
            ...
            if not isinstance(instance["image_num"], list):
                image_num.append(instance["image_num"])
            else:
        ...
        for _images, _image_num in zip(images, image_num):
            assert (
                len(_images) == _image_num
            ), f"Number mismatch between images and placeholder image tokens in 'len(_images) == _image_num'.\
                Expect to have {len(_images)} images but only found {_image_num} images in tokens. \
                Error _image_num: {_image_num}"
        ...
        if len(new_images) > 0:
            ...
            # we still need 2 dummy image for the vision tower
            batch["images"] = torch.zeros(2, 3, crop_size["height"], crop_size["width"])
...
def build_datasets(
    ...
    for dataset in mixture:
        ...
        elif dataset_type == 'torch_geo':
            dataset_cls = LazySupervisedGeoDataset
            if hasattr(dataset, "image_path"):
                image_folder = dataset.image_path
        elif dataset_type == 'torch_levir':
            dataset_cls = LazySupervisedLevirDataset
            if hasattr(dataset, "image_path"):
                image_folder = dataset.image_path
        elif dataset_type == 'torch_era':
            dataset_cls = LazySupervisedEraDataset
            if hasattr(dataset, "image_path"):
                image_folder = dataset.image_path
