...
class LlavaMetaModel(ABC):
    def init_vlm(self, config: PreTrainedModel = None, *args, **kwargs):
        ...
        self.mm_projector = build_mm_projector(mm_projector_cfg, config).to(device=self.vision_tower.device, dtype=self.vision_tower.dtype)
        # self.change_encoder = build_change_encoder(config)
    ...
    def load_pretrained(cls, model_path_or_config, *args, **kwargs):
        ...
        vlm.mm_projector = build_mm_projector(mm_projector_cfg, config).to(device=vlm.vision_tower.device, dtype=vlm.vision_tower.dtype)
    ...
    def freezed_module_patch(self):
        ...
        if self.training:
            ...
            # if getattr(self.config, "tune_cc_projector", True) and not getattr(self.config, "tune_single_projector", True):
            #     self.get_mm_projector().layers.single.eval()
    def encode_images(self, images, image_token_nums: list):
        image_features = self.get_vision_tower()(images)
        image_features = self.get_mm_projector()(image_features, image_token_nums)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, position_ids, attention_mask, past_key_values, labels, images, image_num
    ):
        ...
        image_token_nums = image_num
        image_features = self.encode_images(images, image_token_nums)
