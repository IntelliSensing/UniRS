    ...
    **kwargs,
    ...
    if is_mm_model(model_path):
        ...
        ## TODO @yunhao: mind fixing this
            ...
            prepare_config_for_eval(config, kwargs, args)
            if "mpt" in model_name.lower():
                model = LlavaMPTForCausalLM.from_pretrained(
...
def prepare_config_for_eval(config: PretrainedConfig, kwargs: dict, args=None):
    try:
    ...
    config.mm_projector_cfg["cc_n_layers"] = args.cc_n_layers
    config.mm_projector_cfg["cc_head"] = args.cc_head
    config.mm_projector_cfg["cc_dropout"] = args.cc_dropout
    config.vision_resolution = args.vision_resolution