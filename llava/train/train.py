...
if "WANDB_PROJECT" not in os.environ:
    ...
    os.environ["WANDB_PROJECT"] = "UNIRS_PLUS"
...
def train():
    ...
    if model.get_vision_tower():
        ...
        model.get_mm_projector().layers.change.requires_grad_(training_args.tune_cc_projector)
        model.get_mm_projector().layers.single.requires_grad_(training_args.tune_single_projector)
        mprint(f"vision tower {training_args.tune_vision_tower}")
        mprint(f"mm projector {training_args.tune_mm_projector}")
        mprint(f"cc projector {training_args.tune_cc_projector}")
        mprint(f"single projector {training_args.tune_single_projector}")
